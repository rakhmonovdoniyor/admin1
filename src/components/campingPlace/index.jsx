...
export default function CampingPlaceCom() {
  ...
  const handleSubmit = async (event)=>{
    ...
    if(name !== ''){
      try {
        const response = await axios.post("http://localhost:5052/api/camp-place", {
            name,
...
const DeleteMotor = async (name) => {
  try {
    const response = await axios.delete(`http://localhost:5052/api/camp-place/${name}`);
    console.log(response.data);
    // fetchData();
  } catch (error) {
    console.log(errConsole, error);
  ...
  return (
    <MotoTable>
      ...
      <Box sx={{ width: '100%' }}>
          ...
          <Table borderAxis="bothBetween" stripe="odd" hoverRow sx={{ '& tr > *:first-child': { position: 'sticky', left: 0, bgcolor: 'background.surface' }, '& tr > *:last-child': { position: 'sticky', right: 0, bgcolor: 'var(--TableCell-headBackground)' } }}>
            <thead>
              <tr>
                ...
                <th style={{ width: 200 }}>Place Name</th>
                <th style={{ width: 200 }}>Website Link</th>
        ...
        <CampingModal open={open} setOpen={setOpen} 
