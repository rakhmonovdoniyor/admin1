...
import { BaseCaravanURL, errConsole } from "../config/config";
import { BaseURL } from "../config/config1";
import { AddingContainer, Input } from "../motor/newstyle";
import CaravanModal from "./modalcaravan";

export default function CaravanCom() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(BaseCaravanURL);
  ...
  const handleEdit = async (oldName) => {
    try {
      const response = await axios.put(`${BaseCaravanURL}/${oldName}`, {
        newName,
        newBrand,
        newCost,
        newCompany,
  ...
  //       `http://localhost:5052/api/caravan/${name}`
  //     );
  //     console.log(response.data);
  //     // fetchData();
  //   } catch (error) {
    ...
    <MotoTable>
      ...
      <Box sx={{ width: "100%" }}>
        ...
        <CaravanModal
          ...
          nameCaravan={name}
          brandCaravan={brand}
          companyCaravan={company}
          locationCaravan={location}
          costCaravan={cost}
          typeCaravan={type}
          peopleCaravan={people}
          dateCaravan={date}
          handleSubmit={handleSubmit}
          onCaravanName={handleChange}
          onCaravanBrand={handleBrandChange}
          onCaravanCompany={handleCompanyChange}
          onCaravanCost={handleCostChange}
          onCaravanLocation={handleLocationChange}
          onCaravanType={handleTypeChange}
          onCaravanDate={handleDateChange}
          onCaravanPeople={handlePeopleChange}
        />
      </Box>
    </MotoTable>
  );
